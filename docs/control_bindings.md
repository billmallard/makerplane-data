...
Status: **DRAFT for review** (2026-07-07). **Umbrella** over
...
**When the imperative path still wins:** momentary, stateless actions with no
persistent setting — "recenter now," "clear route." Those are awkward as a stored
value and are better as a command (§10, Phase 4). A map has few of these; almost
everything it exposes is a setting.
...
- **Phase 4 — breadth + the imperative path.** Mark settings bindable across all
  settable instruments; add the complementary *command* verb for momentary,
  stateless actions (the general form of the `setUnits` self-select pattern) for
  the cases a stored value does not fit.

## 11. Open questions / decisions to lock (for Bill / Eric / Phil)

1. **Key namespace + naming.** One flat UI namespace, or per-subsystem (`MAP*`,
   `PFD*`)? Per-instance suffix: reuse the button `{id}` node scheme, or a
   configurator-assigned index? (Drives §6.)
2. **Who defines the keys.** Configurator-generated fixgw include (preferred —
   matches TSBTN auto-alloc) vs a hand-curated database file. Interaction with the
   existing `TSBTN`/`BTN`/`ENC` definitions.
3. **Startup ordering + persistence defaults.** Confirm "config default seeds the
   key; persisted value wins on connect." Should any setting persist by default,
   or is persistence always opt-in?
4. **Bindable vs category-2 (#64).** Gauge **bands / V-speeds** are category-2
   *read-only reference* values on a key's `min`/`max`/`aux` — they are **not**
   this. A bindable **setting** is read-write on the key's *value* channel. Keep
   the two firmly distinct so the layout-vs-FIX boundary (#64) stays clean.
5. **Momentary commands.** FIX trigger-key (write 1, server resets) vs a new
   imperative verb. Which for "recenter"/"clear route"?
6. **Coverage order.** After the map, which instruments get bindable settings
   first (PFD range/minimums? EMS page? the radio, alongside the deferred radio
   work)?
